...
var ChainpointBinary = function () {
  ...
  function _createBinary (proofObject, callback) {
    ...
    if (!appendVersionByteResult) return callback('Could not parse Chainpoint v3 object')
        ...
        // add target_hash
    if (!proofObject.target_hash || !rgxs.isHex(proofObject.target_hash)) return callback('Could not parse Chainpoint v3 object')
    _appendVLQBytes(proofArray, new Buffer(proofObject.target_hash, 'hex'))
  ...
  function _parseBinary (proof, callback) {
    ...
    proofObject.target_hash = targetHashResult[1].toString('hex')
  ...
  function _appendAnchorBytes (proofArray, anchors) {
    ...
    for (var x = 0; x < anchors.length; x++) {
      switch (anchors[x].type) {
        case 'eth':
          if (!anchors[x].source_id || anchors[x].source_id.length !== 64 || !rgxs.isHex(anchors[x].source_id)) return false
          proofArray.push(0xa1)
          _appendVLQBytes(proofArray, new Buffer(anchors[x].source_id, 'hex'))
          break
        case 'btc':
          if (!anchors[x].source_id || !rgxs.isInt(anchors[x].source_id)) return false
          var sourceIdInt = parseInt(anchors[x].source_id, 10)
          ...
          break
        case 'cal':
          if (!anchors[x].source_id) return false
          proofArray.push(0xa3)
          _appendVLQBytes(proofArray, new Buffer(anchors[x].source_id))
          break
        default:
          return false
      }
    ...
    while (currentByte !== 0xf3) {
      ...
      switch (currentByte) {
        case 0xa1:
          ...
          anchor.source_id = sourceIdResult[1].toString('hex')
          break
        case 0xa2:
          anchor.type = 'btc'
          sourceIdResult = _readVLQValue(proof, dataIndex)
          if (!sourceIdResult) return false
          dataIndex = sourceIdResult[0]
          anchor.source_id = sourceIdResult[1].readUInt32BE(0)
          break
        case 0xa3:
          anchor.type = 'cal'
          sourceIdResult = _readVLQValue(proof, dataIndex)
          if (!sourceIdResult) return false
          dataIndex = sourceIdResult[0]
          anchor.source_id = sourceIdResult[1].toString()
          break
        default:
          return false
      }
